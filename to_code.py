...
'fgab1_16':'f1/16 g1/16 a1/16 b1/16','b14_e14_g14':'{b/14 e/14 g/14}','b14_f14_g14' : '{b/14 f/14 g/14}',
'c14_e14_g14':'{c/14 e/14 g/14}' ,'g14_e14':'{g/14 e/14}','#':'#','##':'##','&':'&','&&':'&&','.hi':'.',
'2':'2','4':'4'
 } 
        # retval, img_binary = cv2.threshold(Image,235,255,type=cv2.THRESH_BINARY)
        # dilation2 = cv2.dilate(img_binary,kernel,iterations =50)
        # rect2 = cv2.getStructuringElement(cv2.MORPH_RECT, (60, 50)) #the structure element
        # img_closing = binary_closing(dilation2,rect2)
        # contours2 = find_contours(dilation2,0.8)
        # results2 = []

        # for c in contours2 :
        #     ll2, ur2 = np.min(c, 0), np.max(c, 0) #getting the two points
        #     wh2 = ur2 - ll2  #getting the width and the height
        #     x2,y2,w2,h2 = ll2[0], ll2[1], wh2[1], wh2[0]
        #     results2.append(x2) 
        # imgs_lines.append(results2)
...
def to_code(start,end,lines,folder_name):
    ...
    if code ==  'a/1' or code ==  'a/2' or code ==  'a/4' or code ==  'a/8' or code ==  'a/16' or code ==  'a/32':
        if end <= lines[4]+ 2*space and end >= lines[4]+space :
            code=code.replace('a','c1')
        elif end <= lines[4]+space and end >= lines[4]:
            code=code.replace('a','d1')
        elif end <= lines[4]+0.5*space and end >= lines[3]+0.5*space:
            code=code.replace('a','e1')
        elif end <= lines[4] and end >= lines[3]:
            code=code.replace('a','f1')
        elif end <= lines[3]+0.5*space and end >= lines[2]+0.5*space:
            code=code.replace('a','g1')
        elif end <= lines[3] and end >= lines[2]:
            code=code.replace('a','a1')
    #--------------------------------------------------------------------------------------------------        
    if code ==  'b/1' or code ==  'b/2' or code ==  'b/4' or code ==  'b/8' or code ==  'b/16' or code ==  'b/32':
        if start <= lines[0] - 2*space :
            code=code.replace('b','b2')
        elif start<= lines[0]- 1.5*space and start >= lines[0]- 2*space :
            code=code.replace('b','a2')
        elif start<= lines[0] and start >= lines[0]-space :
            code=code.replace('b','g2')
        elif start<= lines[0]  and start >= lines[0]-0.5*space :
            code=code.replace('b','f2')
        elif start<= lines[1]  and start >= lines[0] :
            code=code.replace('b','e2')
        elif start<= lines[2]-0.5*space  and start > lines[1]-0.5*space :
            code=code.replace('b','d2')
        elif start<= lines[2]  and start >= lines[1] :
            code=code.replace('b','c2')
        elif start<= lines[3]-0.5*space  and start >= lines[2]-0.5*space :
...
def get_similar_temp(template):
    folders = glob.glob("D:/college/cmp3/first/ip/symphony/Symphony/ourdata/*")
    ...
    for pt in zip(*max_loc[::-1]):   #-1 to swap the values as we assign x and y coordinate to draw the rectangle. 
        ...
        cv2.destroyAllWindows()
    return max_path
img_to_compare = cv2.imread("D:/college/cmp3/first/ip/symphony/Symphony/a_8.2r-25.png",0)
path=get_similar_temp(img_to_compare)

code = to_code(12,30,[13,17,21,25,29],path)
